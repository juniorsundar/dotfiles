...
- [x] The Picker interface exists as a bundle naming five parts: Source, Candidate shape, Narrowing, Render, Accept.
- [x] The Candidate contract is `id` + `label` (shared) plus per-type typed fields; a `FileCandidate` extends it with its file-specific fields.
- [x] RowParts is defined as `{ primary, secondary?, icon?, tooltip? }` and is the sole output of a picker's Render.
- [x] PickerContext is defined with host-backed helpers (open text document, reveal position, execute command, read origin) that Accept and Preview act through.
- [x] Source / SourceSession types exist for the snapshot shape (an initial batch of candidates, no updates channel).
- [x] A registry accepts picker registrations by id.
- [x] The file picker is assembled from the prefactored parts (file Source, path-biased Narrowing on the fuzzy primitive, FileCandidate→RowParts Render, Accept and Preview via PickerContext) and registered.
...
- [x] No vscode coupling lives in the Picker core types or the file picker parts; vscode stays in the (future) host and the production PickerContext wiring.