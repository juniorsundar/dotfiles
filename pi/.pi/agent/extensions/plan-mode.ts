...
function markCompletedSteps(text: string, items: TodoItem[]): number {
	...
	for (const match of text.matchAll(/\[DONE:(\d+)\]/gi)) {
		...
		const item = Number.isFinite(step) ? items.find((candidate) => candidate.step === step) : undefined;
...
function requirePlanFileContent(action: "write" | "append", content: string | undefined): string {
	if (typeof content !== "string") throw new Error(`${action} requires a content string.`);
	return content;
}
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const needsSeparator =
				existedBefore && readFileSync(filePath, "utf8").length > 0 && content.length > 0 && !content.startsWith("\n");
...
export default function planModeExtension(pi: ExtensionAPI): void {
	...
	function enterPlanMode(ctx: ExtensionContext): void {
		...
		todoItems = [];
		setPlanTools();
		ctx.ui.notify("Plan mode enabled. I can inspect and draft a plan, but cannot edit until you approve it.", "info");
		updateStatus(ctx);
		persistState();
	}
	...
	pi.registerCommand("plan", {
		...
		handler: async (args, ctx) => {
			...
			if (["off", "stop", "disable", "exit"].includes(mode)) return exitPlanMode(ctx);
			if (mode === "status") {
				const state = executionMode ? "executing" : planModeEnabled ? "planning" : "off";
				ctx.ui.notify(`Plan mode: ${state}. Steps: ${todoItems.length}.`, "info");
				return;
			}
	...
	pi.registerCommand("todos", {
		...
		handler: async (_args, ctx) => {
			if (todoItems.length === 0) {
				ctx.ui.notify("No plan steps tracked yet. Enable /plan and ask for a numbered Plan: first.", "info");
				return;
			}
		...
		return {
			messages: event.messages.filter((message) => {
				...
				return !message.content.some((contentBlock) => {
					if (typeof contentBlock !== "object" || contentBlock === null) return false;
...
After completing each step, include the matching [DONE:n] marker in your response.`,
	...
	pi.on("agent_end", async (event, ctx) => {
		...
		const lastAssistant = [...event.messages].reverse().find((message) => isAssistantMessage(message as MessageLike));
		if (lastAssistant) {
			const extracted = extractTodoItems(getTextContent(lastAssistant as AssistantMessage));
		...
		const choice = await ctx.ui.select("Plan mode - what next?", [
			todoItems.length > 0 ? "Approve and execute plan" : "Exit plan mode and execute",
			"Stay in plan mode",
			"Refine the plan",
			"Exit plan mode without executing",
		]);

		if (choice?.startsWith("Approve") || choice?.startsWith("Exit plan mode and execute")) {
			startExecution(ctx);
			const execMessage =
				todoItems.length > 0
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const legacyEntry = entries
			.filter((entry) => entry.type === "custom" && entry.customType === "plan-mode-toggle")
			.pop() as { data?: { active?: boolean } } | undefined;
		if (!planModeEntry?.data && legacyEntry?.data) planModeEnabled = legacyEntry.data.active ?? planModeEnabled;
		...
		if (executionMode && todoItems.length > 0) {
			...
			for (let i = executeIndex + 1; i < entries.length; i++) {
				...
				if (entry.type === "message" && "message" in entry) {
					...
					if (isAssistantMessage(message as MessageLike)) messages.push(message as AssistantMessage);
				}
			}
