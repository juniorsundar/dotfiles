...
async function inGitRepo(pi: ExtensionAPI): Promise<boolean> {
	const result = await pi.exec("git", ["rev-parse", "--is-inside-work-tree"], { timeout: 5_000 });
...
async function createCheckpoint(pi: ExtensionAPI, label: string): Promise<Checkpoint | undefined> {
	...
	const result = await pi.exec("git", ["stash", "create", label], { timeout: 15_000 });
...
export default function gitCheckpointExtension(pi: ExtensionAPI) {
	...
	async function checkpointNow(ctx: ExtensionContext, label: string): Promise<void> {
		const checkpoint = await createCheckpoint(pi, label);
		if (!checkpoint) {
			if (ctx.hasUI) ctx.ui.notify("No tracked git changes to checkpoint.", "info");
			return;
		}
		remember(checkpoint);
		if (ctx.hasUI) ctx.ui.notify(`Checkpoint created: ${checkpoint.ref.slice(0, 12)}`, "info");
	}

	pi.registerCommand("checkpoint", {
		description: "Create/list/restore lightweight git checkpoints: /checkpoint [list|restore|on|off]",
		handler: async (args, ctx) => {
			...
			if (action.startsWith("restore") || action === "undo") {
				...
				const choice = await ctx.ui.select("Restore checkpoint? This applies a git stash commit to your working tree.", checkpoints.map(formatCheckpoint));
				if (!choice) return;
				const index = Number(choice.split(".")[0]) - 1;
				const checkpoint = checkpoints[index];
				if (!checkpoint) return;
				const ok = await ctx.ui.confirm("Apply checkpoint?", `Run: git stash apply ${checkpoint.ref}\n\nCurrent changes may conflict.`);
				if (!ok) return;
				const result = await pi.exec("git", ["stash", "apply", checkpoint.ref], { timeout: 30_000 });
				ctx.ui.notify(result.code === 0 ? "Checkpoint applied." : `Checkpoint apply failed: ${result.stderr || result.stdout}`, result.code === 0 ? "info" : "error");
				return;
			}
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (event.toolName === "bash") {
			...
			if (!/[>]|\b(rm|mv|cp|mkdir|touch|chmod|chown|git\s+(add|commit|reset|checkout|merge|rebase|stash)|npm\s+(install|ci)|pnpm\s+(install|add)|yarn\s+(install|add))\b/i.test(command)) return;
		}
	...
	pi.on("session_start", async (_event, ctx) => {
		const entry = ctx.sessionManager
			.getEntries()
			.filter((candidate) => candidate.type === "custom" && candidate.customType === "git-checkpoint")
			.pop() as { data?: { enabled?: boolean; checkpoints?: Checkpoint[] } } | undefined;
		if (entry?.data) {
			enabled = entry.data.enabled ?? enabled;
			checkpoints = entry.data.checkpoints ?? checkpoints;
		}
		ctx.ui.setStatus("git-checkpoint", enabled ? ctx.ui.theme.fg("muted", "ckpt:on") : undefined);
	});
}
