...
const ROOT_RULE_FILES = ["AGENTS.md", "CLAUDE.md", "GEMINI.md", "CONVENTIONS.md"];
const RULE_DIRS = [".opencode", ".opencode/rules", ".claude/rules", ".cursor/rules"];
const CONFIG_FILES = ["opencode.json", ".opencode.json", ".opencode/config.json"];
...
function walkMarkdown(dir: string, maxDepth = 3, depth = 0): string[] {
	...
	for (const entry of readdirSync(dir, { withFileTypes: true })) {
		...
		if (entry.isDirectory()) files.push(...walkMarkdown(full, maxDepth, depth + 1));
		else if (entry.isFile() && /\.(md|mdc|txt)$/i.test(entry.name)) files.push(full);
	}
	return files;
}
...
function readRule(root: string, path: string): RuleFile | undefined {
	...
	if (stat.size > MAX_FILE_BYTES) {
		return { path: rel, label: rel, content: `[${rel} omitted: ${stat.size} bytes exceeds ${MAX_FILE_BYTES} byte limit]` };
	}
...
function loadRules(cwd: string): RuleFile[] {
	...
	for (const dir of RULE_DIRS) {
		...
		if (isDir(absoluteDir)) for (const file of walkMarkdown(absoluteDir)) candidates.add(file);
	}
	...
	for (const candidate of [...candidates].sort()) {
		...
		if (total + bytes > MAX_TOTAL_BYTES) {
			rules.push({ path: rule.path, label: rule.label, content: `[${rule.path} omitted: total rules limit reached]` });
			continue;
		}
		total += bytes;
		rules.push(rule);
...
export default function opencodeRulesExtension(pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (rules.length > 0 && ctx.hasUI) ctx.ui.notify(`Loaded ${rules.length} opencode/Claude rule file(s)`, "info");
	});
	...
	pi.registerCommand("rules", {
		...
		handler: async (_args, ctx) => {
			rules = loadRules(ctx.cwd);
			if (rules.length === 0) {
				ctx.ui.notify("No AGENTS.md, CLAUDE.md, .opencode, or .claude/rules files found.", "info");
				return;
			}
			pi.sendMessage({
