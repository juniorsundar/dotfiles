...
const REVIEW_TOOLS = new Set(["read", "bash", "grep", "find", "ls", "web_search", "ask_user_question", "Agent", "get_subagent_result", "steer_subagent"]);
const READ_ONLY_SUBAGENTS = new Set(["explore", "plan", "scout"]);
const SAFE_BASH = /^\s*(git\s+(status|diff|show|log|branch)|rg\b|grep\b|find\b|ls\b|pwd\b|sed\s+-n\b|awk\b|head\b|tail\b|cat\b|wc\b|sort\b|uniq\b|diff\b)/i;
const UNSAFE_BASH = /\b(rm|mv|cp|mkdir|touch|chmod|chown|git\s+(add|commit|push|pull|merge|rebase|reset|checkout|stash)|npm\s+(install|ci)|pnpm\s+(install|add)|yarn\s+(install|add)|sudo)\b|>|>>/i;
...
export default function reviewModeExtension(pi: ExtensionAPI) {
	...
	function update(ctx: ExtensionContext): void {
		ctx.ui.setStatus("review-mode", enabled ? ctx.ui.theme.fg("warning", "review") : undefined);
		ctx.ui.setWidget("review-mode", enabled ? ["🔎 REVIEW MODE: read-only"] : undefined);
	}
	...
	pi.on("tool_call", async (event) => {
		if (!enabled) return;
		if (!REVIEW_TOOLS.has(event.toolName)) return { block: true, reason: `Review mode blocks ${event.toolName}; disable /review-mode to edit.` };
		...
		if (UNSAFE_BASH.test(command) || !SAFE_BASH.test(command)) {
			return { block: true, reason: `Review mode blocks mutating or unrecognized shell command: ${command}` };
		}
	});
	...
	pi.on("session_start", async (_event, ctx) => {
		const entry = ctx.sessionManager
			.getEntries()
			.filter((candidate) => candidate.type === "custom" && candidate.customType === "review-mode")
