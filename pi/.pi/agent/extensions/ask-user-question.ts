...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptSnippet:
      "Ask the user a targeted interactive question and wait for their answer",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      type: StringEnum(["free-form", "single", "multiple"] as const, {
        description:
          "Question style: free-form text, single selection, or multiple selection.",
      }),
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (params.type === "free-form") {
        ...
        if (options.length === 0)
          throw new Error("single selection questions require options");
        ...
        const options = normalizeOptions(params.options);
        if (options.length === 0)
          throw new Error("multiple selection questions require options");
        ...
        answer =
          raw === undefined
            ? undefined
            : parseMultipleAnswer(raw, options).join(", ");
      }

      if (answer === undefined || answer.trim() === "") {
        return {
          content: [
            { type: "text", text: "User cancelled or provided no answer." },
          ],
          details: { cancelled: true },
        };
      }
...
function buildMultipleChoicePrefill(
  options: string[],
  recommended: string[] | undefined,
): string {
