...
function packageScript(cwd: string, names: string[]): string | undefined {
  ...
  try {
    ...
    for (const name of names)
      if (scripts[name]) return `${manager} run ${name}`;
  } catch {
    return undefined;
  }
  return undefined;
...
function detectCommand(cwd: string, kind: string): string | undefined {
  if (kind === "test") {
    return (
      packageScript(cwd, ["test", "tests"]) ??
      (existsSync(join(cwd, "Cargo.toml")) ? "cargo test" : undefined) ??
      (existsSync(join(cwd, "go.mod")) ? "go test ./..." : undefined) ??
      (existsSync(join(cwd, "pyproject.toml")) ? "pytest" : undefined)
    );
  }
  if (kind === "lint") return packageScript(cwd, ["lint", "check:lint"]);
  if (kind === "typecheck")
    return packageScript(cwd, ["typecheck", "type-check", "check", "tsc"]);
...
export default function diagnosticsExtension(pi: ExtensionAPI) {
  ...
  async function runDiagnostic(
    ...
    pi.sendMessage(
      {
        customType: "diagnostic-result",
        content: formatDiagnostic(lastDiagnostic),
        display: true,
      },
      { triggerTurn: false },
    );
    if (sendToAgent && lastDiagnostic.code !== 0) {
      pi.sendUserMessage(
        `Fix the following ${kind} failure. Keep changes focused and rerun the relevant check when done.\n\n${formatDiagnostic(lastDiagnostic)}`,
      );
    }
  }
  ...
  pi.registerCommand("fix-failures", {
    description:
      "Send the last diagnostic failure back to the agent for fixing",
    handler: async (_args, ctx) => {
      ...
      pi.sendUserMessage(
        `Fix this failure. Keep changes focused and rerun checks when done.\n\n${formatDiagnostic(lastDiagnostic)}`,
      );
    },
  });
  ...
  pi.on("session_start", async (_event, ctx) => {
    const entry = ctx.sessionManager
      ...
      .filter(
        (candidate) =>
          candidate.type === "custom" && candidate.customType === "diagnostics",
      )
