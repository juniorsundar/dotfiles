...
globalkeys = gears.table.join(
    ...
    -- Layout manipulation
    awful.key({ modkey, "Shift" }, "Up", function() awful.client.swap.byidx(1) end, { description = "swap with next client by index", group = "client" }),
    awful.key({ modkey, "Shift" }, "Down", function() awful.client.swap.byidx(-1) end, { description = "swap with previous client by index", group = "client" }),
    awful.key({ modkey, "Control" }, "Up", function() awful.screen.focus_relative(1) end, { description = "focus the next screen", group = "screen" }),
    awful.key({ modkey, "Control" }, "Down", function() awful.screen.focus_relative(-1) end, { description = "focus the previous screen", group = "screen" }),
    ...
    awful.key({ modkey }, "r", function() awful.util.spawn("rofi -show drun") end, { description = "Rofi: Run", group = "launcher" }),
    awful.key({ modkey }, "p", function() awful.util.spawn("rofi -show window") end, { description = "Rofi: Windows", group = "launcher" }),
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey }, "n", function(c) c.minimized = true end, { description = "maximise", group = "client" }),
...
for i = 1, 9 do
    globalkeys = gears.table.join(
        ...
        -- Add bindings for numpad keys here
        awful.key({ modkey }, "#" .. np_map[i], function() -- View tag only
            local screen = awful.screen.focused()
            local tag = screen.tags[i]
            if tag then
                tag:view_only()
        ...
        -- Add bindings for numpad keys here
        awful.key({ modkey, "Control" }, "#" .. np_map[i], function() -- View tag only
            local screen = awful.screen.focused()
            local tag = screen.tags[i]
            if tag then
                awful.tag.viewtoggle(tag)
        ...
        -- Add bindings for numpad keys here
        awful.key({ modkey, "Shift" }, "#" .. np_map[i], function() -- View tag only
            if client.focus then
                local tag = client.focus.screen.tags[i]
                if tag then
                    client.focus:move_to_tag(tag)
